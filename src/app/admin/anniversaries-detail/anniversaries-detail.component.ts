...
import { notBlankValidator, notFutureDateValidator } from '../../../libs/validators.utils';

@Component({
  selector: 'app-anniversaries-detail',
  standalone: true,
  providers: [provideNativeDateAdapter()],
  imports: [MatFormFieldModule, MatInputModule, MatButtonModule, TextFieldModule, ReactiveFormsModule, MatDatepickerModule],
...
export class AnniversariesDetailComponent {
  ...
  form = this.fb.group({
    date: this.fb.control<string | null>(null, { validators: [Validators.required, notFutureDateValidator] }),
    title: this.fb.control('', { validators: [Validators.required, notBlankValidator, Validators.minLength(3), Validators.maxLength(200)] }),
    description: this.fb.control('', { validators: [Validators.maxLength(1000)] })
  });
  ...
  get description() { return this.form.controls.description; }
    ...
    async saveDoc() {
      ...
      await setDoc(
        doc( this.firestore, "anniversaries", uuidv4()),
        {
